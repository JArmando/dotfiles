...
set previewwindow winheight=10

set clipboard=unnamedplus
vnoremap Y "*y :echom "copied to tmux press ctrl y"<CR>

function NotifyYank(lines)
  let lineCount = len(v:event.regcontents)
  let message = "yanked " . lineCount
  let lines = " line "
  if lineCount > 1
    let lines =" lines "
  endif
  let blah = message . lines . " | Use Y to yank to tmux then Ctrl to yank to system"
  echom blah
endfunction

autocmd TextYankPost * call NotifyYank(v:event.regcontents)
